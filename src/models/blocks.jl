...
A convolutional block that acts on two arguments `(x, emb)`. Its output is
`activation(norm(conv(x)) .+ embed_layers(emb))` where `embed_layers` is shaped so that each
value in the embedding channels is mapped to one output channel.
"""
...
function ConvEmbed(
    ...
    out = channels[2]
    embed_layers = Chain(swish, Dense(emb_channels, out))
...
function ResBlock(
    ...
    out_ch = channels[2]
    conv_timestep =
        ConvEmbed(channels, emb_channels; groups = groups, activation = activation)
    out_layers = Chain(
