...
A convolutional autoencoder with time embeddings, class embeddings and skip connections. The
default configuration has 17 layers and skip connections (each ResBlock and upsample has 2
...
function UNetConditioned(
    ...
    model_channels % block_groups == 0 || error(
        "The number of block_groups ($(block_groups)) must divide the number of" *
    ...
    UNetConditioned(
        ...
        combine_embeddings,
        chain,
        length(channel_multipliers) + 1,
    )
end

function (u::UNetConditioned)(
    x::AbstractArray,
    ...
    if (size(x, 1) % downsize_factor != 0) || (size(x, 2) % downsize_factor != 0)
        throw(
            DimensionMismatch(
                "image size $(size(x)[1:2]) is not divisible by $downsize_factor" *
                " which is required for concatenation during upsampling.",
            ),
        )
    end
