...
loss(diffusion, x) =
    p_losses(diffusion, loss_type, x; to_device = to_device, p_uncond = p_uncond)
...
X0_all =
    p_sample_loop(diffusion, collect(1:11); guidance_scale = 2.0f0, to_device = to_device);
X0_all = X0_all |> cpu;
imgs = convert2image(trainset, X0_all[:, :, 1, :]);
p_all = plot([plot(imgs[:, :, i], title = "digit=$(i-2)") for i = 1:11]..., ticks = nothing)
display(p_all)
...
for label = 1:11
    ...
    p0 = plot(
        [plot(imgs[:, :, i]) for i = 1:12]...,
        plot_title = "label=$label",
        ticks = nothing,
    )
    display(p0)
end
